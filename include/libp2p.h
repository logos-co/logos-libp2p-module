...
// serviceData may be NULL (dataLen == 0).
...
// Looks up advertisements for a service. The callback receives an array of
// Libp2pExtendedPeerRecord valid only for the duration of the callback.
// serviceData may be NULL (dataLen == 0).
...
// Returns peer records encountered while walking toward a random peer ID.
